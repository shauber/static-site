...
resource "azurerm_storage_account" "static-site-sa" {
  name                      = "${var.env}0${substr(replace(local.safe-domain-name,"-", ""),0,(24-length(var.env)-length(random_string.random_string.suffix.result)))}0${random_string.suffix.result}"
